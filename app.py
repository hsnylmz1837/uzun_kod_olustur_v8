...
with header:
    ...
    with right:
        try:
            st.image("data/coiltech_logo.png", use_column_width=True)
        except Exception:
            pass
...
def is_skip_value(code, label):
    c = norm(code); l = norm(label)
    if c in {"yok","diğer","diger","xxx","none","-"}: return True
    if l in {"yok","diğer","diger","-"}: return True
    return False
        ...
        for _, fld in fields.iterrows():
            ...
            if typ == "select":
                ...
                if pd.notna(fld.get("OptionsKey")):
                    row = optdf.query("OptionsKey == @fld.OptionsKey and ValueCode == @val")
                    if not row.empty: label = row.iloc[0]["ValueLabel"]
                if is_skip_value(val, label): continue
                ...
                if pd.notna(fld.get("OptionsKey")):
                    ...
                    for v in val:
                        lbl = ""
                        rr = subset[subset["ValueCode"].astype(str) == str(v)]
                        if not rr.empty: lbl = rr.iloc[0]["ValueLabel"]
                        if not is_skip_value(v, lbl): clean.append(v)
                    ordered = sorted(clean, key=lambda v: order_map.get(str(v), 999999))
                    if ordered: parts.append("".join([sanitize(v) for v in ordered]))
                else:
                    parts.append("".join([sanitize(v) for v in val]))
            elif typ == "number":
        ...
        if secs.empty: st.warning("Bu makine için 'sections' sayfasında kayıt yok.")
                    ...
                    for _, fld in fields.iterrows():
                        ...
                        if typ in ("select", "multiselect"):
                            ...
                            if typ == "select":
                                ...
                                sel = st.selectbox(label + (" *" if req else ""), options=opts_codes, format_func=lambda c: opts_labels[opts_codes.index(c)], index=idx, key=f"k_{k}", disabled=not enabled)
                                if enabled: st.session_state["form_values"][k] = sel
                            else:
                                default_vals = [default] if isinstance(default, str) and default in opts_codes else []
                                ms = st.multiselect(label + (" *" if req else ""), options=opts_codes, default=default_vals, format_func=lambda c: opts_labels[opts_codes.index(c)], key=f"k_{k}", disabled=not enabled)
                                if enabled: st.session_state["form_values"][k] = ms
                        elif typ == "number":
                            ...
                            txt = st.text_input(label + (" *" if req else ""), value=str(default) if pd.notna(default) else "", key=f"k_{k}", disabled=not enabled)
                            if enabled: st.session_state["form_values"][k] = txt
            st.markdown("---")
            ...
            with c2:
                if "long_code" in st.session_state and st.session_state["long_code"]:
                    ...
                    img = qrcode.make(code); buf = io.BytesIO(); img.save(buf, format="PNG"); st.image(buf.getvalue(), caption="QR"); st.download_button("Kodu TXT indir", data=code.encode("utf-8"), file_name="uzun_kod.txt")
